...
pub fn _ownership(){
    ...
    //in case of strings
    //string are stored in the heap or binary read only memory,while 3 parts are stored on stack??not sure (fact check this) -> fact checked, approved
    //string is made of 3 parts:pointer to mem,length(amt of mem.) and capacity.Imagine kinda a like linked list.

    //Binary read only memory ->  it is the memory region allocated to store static datas like string slice.
    //however small static datas like integers can be stored in stack.
   {
     let s1 = String::from("hello");
    let _s2 = s1; //s2 is the new owner of the data now
    //println!("{s1},this") //doing this will throw error though.
}
//here in case of string s2 ,it copies all three parts of string but it does not copy the data on heap which pointer referes to.
